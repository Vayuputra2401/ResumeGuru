...
st.set_page_config(
    page_title="Get Your Resume analyzed ;) ",
    page_icon='./Logo/resume.png',
)


def run():
    st.title("Your very own Resume Analyser ;) ")
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                ds_keyword = ['tensorflow', 'keras', 'pytorch', 'machine learning', 'deep learning', 'flask',
                              'streamlit']
                ds_web_keyword = ['tensorflow', 'keras', 'pytorch', 'machine learning', 'deep Learning', 'flask',
                                  'streamlit', 'react', 'django', 'node jS', 'react js', 'php', 'laravel', 'magento',
                                  'wordpress',
                ...
                for i in resume_data['skills']:
                    ...
                    if i.lower() in ds_keyword:
                        ...
                        st.markdown(
                            '''<h4 style='text-align: left; color: #1ed760;'>Adding these skills to resume will 
                    ...
                    # Web development recommendation
                        ...
                        break
                    # combined
                    elif i.lower() in ds_web_keyword:
                        print(i.lower())
                        reco_field = 'Data Science and Web Development'
                        st.success("** Our analysis says you are looking for both Data Science and Web Development "
                                   "Jobs **")
                        recommended_skills = ['Data Visualization', 'Predictive Analysis', 'Statistical Modeling',
                                              'Data Mining', 'Clustering & Classification', 'Data Analytics',
                                              'Quantitative Analysis', 'Web Scraping', 'ML Algorithms', 'Keras',
                                              'Pytorch', 'Probability', 'Scikit-learn', 'Tensorflow', "Flask",
                                              'Streamlit''React', 'Django', 'Node JS', 'React JS', 'php', 'laravel',
                                              'Magento',
                                              'wordpress', 'Javascript', 'Angular JS', 'c#', 'Flask', 'SDK']
                        recommended_keywords = st_tags(label='### Recommended skills for you.',
                                                       text='Recommended skills generated from System',
                ...
                resume_vid = 'https://youtu.be/y8YH0Qbu5h4'
                ...
                interview_vid = 'https://youtu.be/2HQmjLu-6RQ'
        ...
        if st.button('Login'):
            if ad_user == 'pathikreet' and ad_password == '12345':
                st.success("Welcome Pathikreet")
                # Display Data
                ...
                label_counts = plot_data['Predicted_Field'].value_counts().reset_index()
                label_counts.columns = ['Predicted_Field', 'count']

                # Create the pie chart
                st.subheader("📈 **Pie-Chart for Predicted Field Recommendations**")
                fig = px.pie(label_counts, values='count', names='Predicted_Field',
                             title='Predicted Field according to the Skills')
                st.plotly_chart(fig)

                # Get the unique labels and their counts
                label_counts = plot_data['User_level'].value_counts().reset_index()
                label_counts.columns = ['User_level', 'count']

                # Create the pie chart
                st.subheader("📈 **Pie-Chart for Experience of users**")
                fig = px.pie(label_counts, values='count', names='User_level',
                             title='Experience according to the profile')
                st.plotly_chart(fig)
